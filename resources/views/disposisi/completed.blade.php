<x-app-layout>
    <x-slot name="title">Disposisi Selesai</x-slot>
    ...
    <div class="flex bg-primary shadow-sm">
        <div class="py-7 px-6 flex items-center justify-between text-white w-full">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-3xl font-bold mr-4">Disposisi Selesai</h1>
            </div>
        </div>
    </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr>
                            ...
                            <x-table-header>
                                Tanggal Selesai</x-table-header>
                            <x-table-header>
                                Catatan</x-table-header>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($dispositions as $disposition)
                            <tr>
                                ...
                                <x-table-cell
                                    class="text-center whitespace-nowrap">{{ $disposition->completed_at ? $disposition->completed_at->format('d/m/Y H:i') : '-' }}</x-table-cell>
                                <x-table-cell
                                    class="text-center whitespace-nowrap">{{ $disposition->notes ?? '-' }}</x-table-cell>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-5 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        <h3 class="text-lg font-medium text-gray-900">Tidak ada disposisi selesai</h3>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
